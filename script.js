...
function displaySongInfo() {
  ...
  for (let i = 0; i < imageArray.length; i++) {
    imageDisplay.insertAdjacentHTML('beforeend', `<img src=${imageArray[i]}>`);
  }

  for (let i = 0; i < songArray.length; i++) {
    songDisplay.insertAdjacentHTML('beforeend', `<p>${songArray[i]}</p>`)
  }

  for (let i = 0; i < artistsArray.length; i++) {
    artistDisplay.insertAdjacentHTML('beforeend', `<p>${artistsArray[i]}</p>`)
  }

  for (let i = 0; i < linkArray.length; i++) {
    linkDisplay.insertAdjacentHTML('beforeend', `<a href=${linkArray[i]}>Link</a>`)
  }
